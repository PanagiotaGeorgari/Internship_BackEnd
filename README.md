...
|------------------------|------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| **getById**            | Το cardId ανήκει στο currentUser(ADMIN) και ανήκει στο repo                  | Επιστρέφει `GetByIdResponse` με τις κάρτες που έχει ήδη Assocs                           |
|                        | Το cardId ανήκει στο currentUser(MEMBER) και ανήκει στο repo                 | Επιστρέφει `GetByIdResponse` με τις κάρτες που έχει ήδη Assocs                           |
...
| **deleteCard**         | Το cardId ανήκει στο currentUser (Admin) και ανήκει στο repo                 | Διαγράφει την κάρτα από το repo και επιστρέφει void                                      |
|                        | Το cardId ανήκει στο currentUser (Member) και ανήκει στο repo                | Διαγράφει την κάρτα από το repo και επιστρέφει void                                      |
|                        | Το cardId δεν ανήκει στο repo                                                | `Throw new CardNotFoundException()`                                                      |
|                        | Το cardId υπάρχει στο repo, ο current user είναι Member και δεν έχω πρόσβαση | `Throw new AccessDeniedException()`                                                      |
...
| **getCardAvailAssoc**  | Αν ο currentUser είναι Admin                                                 | Παίρνει όλες τις κάρτες εκτός της ίδιας και φιλτράρει τις ήδη υπάρχουσες assocs          |
|                        | Αν ο currentUser είναι Member και η κάρτα του ανήκει                         | Παίρνει μόνο τις δικές του κάρτες και φιλτράρει                                          |    
|                        | Αν ο current user είναι Member και η κάρτα δεν του ανήκει                    | `Throw new AccessDeniedException()`                                                      |
